...
def HnpSolver(label, dh, b, t, beta, fudge):
  ...
  pk_a = power_mod(g, sk_a, p)  # A modifier génération de la clé publique a
  sk_b = ZZ.random_element(1, q)
  pk_b = power_mod(g, sk_b, p)  # A modifier génération de la clé publique b 

  # shared secret
  dh_ab = power_mod(pk_a, sk_b, p) # A modifier génération du secret
  ...
  for i in range(t):
    B[  0, i+2] = factor * mult[i]    # A modifier
    B[  1, i+2] = factor * appr[i]    # A modifier
    B[i+2, i+2] = factor * p          # A modifier
...
experiments = [
  (dh20, 8, 180, 60, fudges[0]),
  (dh50, 8, 180, 60, fudges[0]),
  (dh100, 8, 180, 60, fudges[0])
]
